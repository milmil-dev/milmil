...
function MenuRow({ icon, label, value, onClick }: {
  ...
  return (
    <button type="button" onClick={onClick}
      className="w-full flex items-center gap-2.5 px-3.5 py-2 text-left text-[11px]
        text-white/80 hover:bg-white/[0.06] active:scale-[0.99] transition-all duration-100">
      {icon && <span className="shrink-0 w-4 h-4 text-white/40">{icon}</span>}
      <span className="flex-1 truncate">{label}</span>
      {value && <span className="shrink-0 text-white/40 text-[10px]">{value}</span>}
      <svg viewBox="0 0 24 24" fill="currentColor" className="shrink-0 w-3.5 h-3.5 text-white/20">
...
function OptionRow({ label, selected, onClick, swatch }: {
  ...
  return (
    <button type="button" onClick={onClick}
      className={cn(
        'w-full flex items-center gap-2.5 px-3.5 py-2 text-left text-[11px] transition-all duration-100 active:scale-[0.99]',
        selected ? 'text-white bg-white/[0.08]' : 'text-white/60 hover:bg-white/[0.06]',
      )}>
...
function BackHeader({ title, onBack }: { title: string; onBack: () => void }) {
  return (
    <button type="button" onClick={onBack}
      className="w-full flex items-center gap-2 px-3.5 py-2 text-left text-[11px] font-medium
        text-white/80 border-b border-white/[0.06] hover:bg-white/[0.06] transition-colors">
  ...
  return (
    <div className="px-3.5 py-2.5 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-white/50 tabular-nums">{display}</span>
      </div>
      <input type="range" min={min} max={max} step={step} value={value}
        onChange={(e) => onChange(Number(e.target.value))}
        className="w-full h-[3px] bg-white/10 rounded-full appearance-none
      ...
      case 'subtitle-style':
        return (<>
          ...
          <div className="flex items-center gap-2.5 px-3.5 py-2">
            <span className="flex-1 text-[11px] text-white/70">Respect ASS Styles</span>
            <button type="button" onClick={() => updateSubStyle({ respectAssStyle: !(subStyle.respectAssStyle ?? true) })}
              className={cn('relative w-8 h-[18px] rounded-full transition-colors',
                (subStyle.respectAssStyle ?? true) ? 'bg-white/25' : 'bg-white/10')}>
              <span className={cn('absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white transition-transform',
                (subStyle.respectAssStyle ?? true) ? 'translate-x-[14px]' : 'translate-x-0.5')} />
            </button>
          </div>
        </>);
    ...
    <>
      ...
      <motion.div
        ...
        className="absolute right-2 bottom-12 z-[101] w-[260px] max-h-[65vh]
          rounded-lg border border-white/[0.08] bg-[#0c0c0c]/95 backdrop-blur-md
          shadow-[0_4px_24px_rgba(0,0,0,0.6)] overflow-hidden"
      >
