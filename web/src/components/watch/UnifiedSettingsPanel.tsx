    ...
    <div className="shrink-0 shadow-[0_1px_0_rgba(255,255,255,0.06)]">
      ...
      >
        <svg
          viewBox="0 0 24 24"
          fill="currentColor"
          className="h-3.5 w-3.5 text-white/50 group-hover:text-white/80 transition-colors"
        >
...
export function UnifiedSettingsPanel({
  ...
  const updateSubStyle = useCallback(
  ...
  // Media state
  const filters = mediaPlugin?.getFilterState() ?? {
    brightness: 100,
    contrast: 100,
    saturation: 100,
    warmth: 0,
  ...
  const updateFilter = useCallback(
    (key: keyof VideoFilterState, v: number) => {
      const setters: Record<keyof VideoFilterState, ((v: number) => void) | undefined> = {
        brightness: mediaPlugin?.setBrightness,
        contrast: mediaPlugin?.setContrast,
        saturation: mediaPlugin?.setSaturation,
        warmth: mediaPlugin?.setWarmth,
      };
      setters[key]?.(v);
      refresh();
    },
  ...
  const headerTitle = ((): string | null => {
    switch (view) {
      case 'main':
        return null;
      case 'subtitles':
        return i18n._(msg`settings.subtitles`);
      case 'subtitle-style':
        return i18n._(msg`settings.subtitleStyle`);
      case 'sub-preset':
        return i18n._(msg`settings.preset`);
      case 'sub-font':
        return i18n._(msg`settings.font`);
      case 'sub-fontSize':
        return i18n._(msg`settings.fontSize`);
      case 'sub-color':
        return i18n._(msg`settings.textColor`);
      case 'sub-bgOpacity':
        return i18n._(msg`settings.background`);
      case 'sub-border':
        return i18n._(msg`settings.borderStyle`);
      case 'sub-position':
        return i18n._(msg`settings.position`);
      case 'sub-timing':
        return i18n._(msg`settings.subtitleTiming`);
      case 'speed':
        return i18n._(msg`settings.playbackSpeed`);
      case 'brightness':
        return i18n._(msg`settings.brightness`);
      case 'contrast':
        return i18n._(msg`settings.contrast`);
      case 'saturation':
        return i18n._(msg`settings.saturation`);
      case 'warmth':
        return i18n._(msg`settings.nightMode`);
      case 'volumeBoost':
        return i18n._(msg`settings.volumeBoost`);
      case 'audioTrack':
        return i18n._(msg`settings.audioTrack`);
    }
  })();
    ...
    switch (view) {
      ...
      case 'subtitles':
        return (
          <>
            ...
            <OptionRow
              label={i18n._(msg`settings.off`)}
              selected={!subPrimary}
              onClick={() => {
                subtitlePlugin?.setPrimary(-1);
      ...
      case 'subtitle-style':
        return (
          <>
            ...
            <MenuRow
              label={i18n._(msg`settings.preset`)}
              value={presetLabel}
              onClick={() => go('sub-preset')}
            />
            <MenuRow
              label={i18n._(msg`settings.font`)}
              value={
                labels.fonts.find((f) => f.value === (subStyle.fontFamily ?? ''))?.label ??
                i18n._(msg`settings.default`)
              }
              onClick={() => go('sub-font')}
            />
            <MenuRow
              label={i18n._(msg`settings.fontSize`)}
              value={`${subStyle.fontSize ?? 24}px`}
              onClick={() => go('sub-fontSize')}
            />
            <MenuRow
              label={i18n._(msg`settings.textColor`)}
              value={labels.colors.find((c) => c.value === (subStyle.color ?? '#FFFFFF'))?.label}
              onClick={() => go('sub-color')}
            />
            <MenuRow
              label={i18n._(msg`settings.background`)}
              value={`${Math.round((subStyle.backgroundOpacity ?? 0.75) * 100)}%`}
              onClick={() => go('sub-bgOpacity')}
            />
            <MenuRow
              label={i18n._(msg`settings.border`)}
              value={labels.shadows.find((s) => s.value === (subStyle.shadowType ?? 'none'))?.label}
              onClick={() => go('sub-border')}
            />
            <MenuRow
              label={i18n._(msg`settings.position`)}
              value={
                labels.positions.find((p) => p.value === (subStyle.position ?? 'bottom'))?.label
              }
              onClick={() => go('sub-position')}
            />
            <Divider />
            <div className="flex items-center gap-2.5 mx-2 px-2.5 py-2">
              <span className="flex-1 text-xs text-white/70">
                {i18n._(msg`settings.respectAssStyles`)}
              </span>
              <button
                type="button"
                onClick={() =>
                  updateSubStyle({ respectAssStyle: !(subStyle.respectAssStyle ?? true) })
                }
                className={cn(
      ...
      case 'sub-preset':
        return (
          <>
            <BackHeader
              title={i18n._(msg`settings.preset`)}
              onBack={() => back('subtitle-style')}
            />
            {labels.presets.map((o) => (
              <OptionRow
                key={o.value}
                label={o.label}
                selected={subPreset === o.value}
                onClick={() => {
                  subtitlePlugin?.setPreset(o.value);
                  setSubStyle({});
                  refresh();
                  back('subtitle-style');
                }}
          ...
          <>
            ...
            {labels.fonts.map((o) => (
              <OptionRow
                key={o.value}
                label={o.label}
                selected={(subStyle.fontFamily ?? '') === o.value}
                onClick={() => {
                  updateSubStyle({ fontFamily: o.value || undefined });
                  back('subtitle-style');
                }}
              />
            ))}
      ...
      case 'sub-fontSize':
        return (
          <>
            <BackHeader
              title={i18n._(msg`settings.fontSize`)}
              onBack={() => back('subtitle-style')}
            />
            {SIZE_OPTIONS.map((s) => (
              <OptionRow
                key={s}
                label={`${s}px${s === 24 ? ' (Default)' : ''}`}
                selected={(subStyle.fontSize ?? 24) === s}
                onClick={() => {
                  updateSubStyle({ fontSize: s });
                  back('subtitle-style');
                }}
              />
            ))}
      ...
      case 'sub-color':
        return (
          <>
            <BackHeader
              title={i18n._(msg`settings.textColor`)}
              onBack={() => back('subtitle-style')}
            />
            {labels.colors.map((o) => (
              <OptionRow
                key={o.value}
                label={o.label}
                selected={(subStyle.color ?? '#FFFFFF') === o.value}
                swatch={o.value}
                onClick={() => {
                  updateSubStyle({ color: o.value });
                  back('subtitle-style');
                }}
              />
            ))}
      ...
      case 'sub-bgOpacity':
        return (
          <>
            <BackHeader
              title={i18n._(msg`settings.background`)}
              onBack={() => back('subtitle-style')}
            />
            {OPACITY_OPTIONS.map((o) => (
              <OptionRow
                key={o}
                label={`${Math.round(o * 100)}%${o === 0.75 ? ' (Default)' : ''}`}
                selected={(subStyle.backgroundOpacity ?? 0.75) === o}
                onClick={() => {
                  updateSubStyle({ backgroundOpacity: o });
                  back('subtitle-style');
                }}
              />
            ))}
      ...
      case 'sub-border':
        return (
          <>
            <BackHeader
              title={i18n._(msg`settings.borderStyle`)}
              onBack={() => back('subtitle-style')}
            />
            {labels.shadows.map((o) => (
              <OptionRow
                key={o.value}
                label={o.label}
                selected={(subStyle.shadowType ?? 'none') === o.value}
                onClick={() => {
                  updateSubStyle({ shadowType: o.value });
                  back('subtitle-style');
                }}
              />
            ))}
      ...
      case 'sub-position':
        return (
          <>
            <BackHeader
              title={i18n._(msg`settings.position`)}
              onBack={() => back('subtitle-style')}
            />
            {labels.positions.map((o) => (
              <OptionRow
                key={o.value}
                label={o.label}
                selected={(subStyle.position ?? 'bottom') === o.value}
                onClick={() => {
                  updateSubStyle({ position: o.value });
                  back('subtitle-style');
                }}
              />
            ))}
        ...
        return (
          <>
            ...
            <SliderControl
              ...
              max={10}
              step={0.1}
              display={`${subDelay >= 0 ? '+' : ''}${subDelay.toFixed(1)}s`}
              defaultVal={0}
              onChange={(v) => {
                subtitlePlugin?.setDelay(v);
                onSubtitleDelayChange?.(v);
                refresh();
              }}
            />
          </>
          ...
          <>
            ...
            <div className="px-3.5 py-3 space-y-4">
              ...
              <div className="flex items-center gap-2.5">
                <button
                  type="button"
                  onClick={() => setSpeed(Math.max(0.25, currentSpeed - 0.25))}
                ...
                <button
                  type="button"
                  onClick={() => setSpeed(Math.min(4, currentSpeed + 0.25))}
                  className="w-7 h-7 flex items-center justify-center rounded-full bg-white/[0.08] text-white/60 hover:bg-white/[0.12] active:scale-95 transition-all text-sm font-medium"
                >
                  +
                </button>
      ...
      case 'brightness':
        return (
          <>
            ...
            <SliderControl
              label={i18n._(msg`settings.brightness`)}
              value={filters.brightness}
              min={0}
              max={200}
              step={5}
              display={`${filters.brightness}%`}
              defaultVal={100}
      ...
      case 'contrast':
        return (
          <>
            ...
            <SliderControl
              label={i18n._(msg`settings.contrast`)}
              value={filters.contrast}
              min={0}
              max={200}
              step={5}
              display={`${filters.contrast}%`}
              defaultVal={100}
      ...
      case 'saturation':
        return (
          <>
            ...
            <SliderControl
              label={i18n._(msg`settings.saturation`)}
              value={filters.saturation}
              min={0}
              max={200}
              step={5}
              display={`${filters.saturation}%`}
              defaultVal={100}
            ...
            {audioTracks.map((t) => {
              ...
              return (
                <OptionRow
                  ...
                  selected={t.enabled}
                  onClick={() => {
                    mediaPlugin?.setAudioTrack(t.index);
                    onAudioChange?.(t.language ?? '');
                    refresh();
                    back('main');
                  }}
                />
          ...
          <>
            ...
            {subtitlePlugin && (
              <>
                <MenuRow
                  icon={<IconSub />}
                  label={i18n._(msg`settings.subtitles`)}
                  value={subTrackLabel}
                  onClick={() => go('subtitles')}
                />
                <MenuRow
                  icon={<IconStyle />}
                  label={i18n._(msg`settings.subtitleStyle`)}
                  value={presetLabel}
                  onClick={() => go('subtitle-style')}
                />
                <MenuRow
                  icon={<IconTimer />}
                  label={i18n._(msg`settings.subtitleTiming`)}
                  value={
                    subDelay !== 0
            ...
            <MenuRow
              icon={<IconSpeed />}
              label={i18n._(msg`settings.playbackSpeed`)}
              value={
                videoEl && videoEl.playbackRate !== 1
                  ? `${videoEl.playbackRate}x`
                  : i18n._(msg`settings.normal`)
              }
              onClick={() => go('speed')}
            />
            ...
            {mediaPlugin && (
              <>
                ...
                <MenuRow
                  ...
                  onClick={() => go('saturation')}
                />
                <MenuRow
                  icon={<IconNight />}
                  label={i18n._(msg`settings.nightMode`)}
                ...
                <MenuRow
                  icon={<IconVolume />}
                  label={i18n._(msg`settings.volumeBoost`)}
                  value={volume !== 100 ? `${volume}%` : undefined}
                  onClick={() => go('volumeBoost')}
                />
                {audioTracks.length > 1 && (
                  <MenuRow
                    icon={<IconAudio />}
                    label={i18n._(msg`settings.audioTrack`)}
                    value={(() => {
                      const cur = audioTracks.find((t) => t.enabled);
                      if (!cur) return undefined;
                      return (
                        formatLanguage(cur.language, i18n.locale ?? 'en') ||
                        cur.label ||
                        cur.language
                      );
                    })()}
    ...
    <>
      ...
      >
        {headerTitle && (
          <PanelHeader
            title={headerTitle}
            onBack={() => back(VIEW_PARENT[view as Exclude<View, 'main'>])}
          />
        )}
