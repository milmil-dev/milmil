...
export function EpisodeRowMisc({
  ...
  return (
    <div className="group grid grid-cols-[1fr_80px_auto_auto] gap-4 items-center px-3 py-2 rounded-md hover:bg-ink/[0.02]">
      ...
      <div className="text-[11px] text-ink/45 tabular-nums min-w-[110px] text-right">
        {status === 'complete' ? fmt(totalBytes) : `${fmt(downloadedBytes)} / ${fmt(totalBytes)}`}
      </div>
      <div className="flex items-center gap-2">
        <span className="text-[11px] text-ink/65 tabular-nums w-[34px] text-right font-medium">
          {percent}%
        </span>
        <button
          type="button"
          ...
          className="text-ink/25 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"
        >
