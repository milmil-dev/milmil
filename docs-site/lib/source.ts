...
// One page per OpenAPI tag, as virtual files under content/docs/api — nothing is written to
// disk. The folder's title and ordering come from the real (and translated)
// content/docs/api/meta*.json, so the section reads like the rest of the docs.
const apiReference = await openapi.staticSource({ baseDir: 'api', per: 'tag' });
